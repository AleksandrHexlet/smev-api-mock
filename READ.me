...
**URL:**
`POST /api/check/v1/CheckPassport`
...
Если в ответе приходит `500`, проверьте контроллер.
Не нужно передавать строку `"300"` или `"301"` в `HttpStatus.valueOf(...)`, потому что Spring не воспринимает такие значения как enum-статусы.
